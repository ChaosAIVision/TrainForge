...
class BaseModelLoader(ABC):
    """Abstract base class for VLM model loaders."""
    
    @abstractmethod
    def load_model_and_tokenizer(self, config: ModelConfig) -> Tuple[PreTrainedModel, PreTrainedTokenizer]:
        """Load VLM model and tokenizer based on configuration."""
        pass


class BaseDatasetLoader(ABC):
    """Abstract base class for VLM dataset loaders."""
    
    @abstractmethod
    def load_dataset(self, config: DatasetConfig) -> Tuple[Dataset, Dataset]:
        """Load VLM train and validation datasets based on configuration."""
        pass


class HuggingFaceDatasetLoader(BaseDatasetLoader):
    """HuggingFace VLM dataset loader implementation."""
    
    def load_dataset(self, config: DatasetConfig) -> Tuple[Dataset, Dataset]:
        """Load VLM dataset from HuggingFace Hub."""
...
class LoaderFactory:
    """Factory class for creating VLM loaders."""
    
    @staticmethod
    def create_dataset_loader(loader_type: str = "huggingface") -> BaseDatasetLoader:
        """Create VLM dataset loader based on type."""
